...
A Tool whose detect dir exists under the operator's home and is not a **skills-only footprint** — a detect dir holding nothing but the path down to the Tool's skills dir (`~/.kiro/skills/x` and nothing else), which is what a skill deployer leaves behind (`npx skills add`, ego-browser, Skills Hub itself once the Tool is uninstalled), not evidence of the Tool. An empty detect dir counts; anything beside the skills path counts. A virtual group's dir is the convention itself (`~/.agents` legitimately holds only `skills/`), so for a group entry presence alone counts. One rule (`tool_adapters::is_installed_in`), read by the Tool catalog, global sync's skip rule and the onboarding scan's `Installed` scope.
