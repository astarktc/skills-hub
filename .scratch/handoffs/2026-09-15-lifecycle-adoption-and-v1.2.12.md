...
- **CI (`ci.yml`) was re-enabled** after five months disabled by hand. Its first run fires on the push that carries this
  note. If the Rust job is red, BACKLOG #01 is the next step — read the failing test before deciding.
...
1. Check the CI run on `main` (`gh run list --workflow ci.yml --limit 1`). Green → delete the parenthetical about the
   disable from AGENTS.md "Commands" and close BACKLOG #01 in the same commit. Red → BACKLOG #01.
2. Then pick from BACKLOG "Now": #02 (wave C) is the largest; #06/#07/#08 are small, related, and could form one
