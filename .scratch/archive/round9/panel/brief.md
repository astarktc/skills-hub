...
Repo: `~/Projects/skills-hub` (Tauri 2 + React 19; Rust `src-tauri/src`, TS `src`). Branch `main` @ `96cd33c` (v1.2.8). **READ ONLY**: do not edit, commit, run mutating commands, launch the app (`npm run tauri:dev` mutates the operator's real skill library), or `git add` anything. `cargo test --all`, `npm test`, `npm run build` are fine to confirm something. Do not search outside this directory (cloud-synced folders). AGENTS.md Workflow step 1 does not apply — start immediately.
...
## Output
Write to `~/Projects/skills-hub/.scratch/round9/panel/<your-file>.md` incrementally AND return it as the final message. 4–8 candidates, ranked. Per candidate:
