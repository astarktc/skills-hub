...
impl SyncStatus {
    ...
    /// "Was a target *ever* deployed", so reconcile can tell "it vanished"
    /// (news) from "it was never there": `Missing` is in because the row was
    /// deployed before it vanished, `Error` is out because a row that failed
    /// on its first sync never had a target to lose.
    ///
    /// There is deliberately **no** status gate for "might bytes still be at
    /// the target now". Artifact removal asks the filesystem instead — one
    /// presence rule (`symlink_metadata`) for every scope — because a status
    /// is a record of the last write, not of what is on disk now
    /// (`artifact_removal`, ADR-0002).
